...
export default async function ShopPage() {
  ...
  const itemsWithColors = shopItems.map((item: { 
    ...
    color: item.category === 'power-ups' 
      ...
      : item.icon === 'Lightbulb' ? 'text-amber-500'
      : item.icon === 'Gift' ? 'text-pink-500'
      : item.icon === 'Clock' ? 'text-blue-500'
      : item.icon === 'Sparkles' ? 'text-purple-500'
      : item.icon === 'Shield' ? 'text-indigo-500'
      : item.icon === 'Flame' ? 'text-orange-500'
      : item.icon === 'FastForward' ? 'text-teal-500'
      : item.icon === 'Brain' ? 'text-fuchsia-500'
      : 'text-amber-500'
